...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM `admin_registration` WHERE `Email` = ? AND `Password` = ?";
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($result->num_rows == 1) {
            ...
            $_SESSION['Email'] = $row['Email']; // Ensure the column name matches the database
            
            // Check role and redirect accordingly
            if ($row['Role'] == 'Admin') {
                header('Location: index.php');
            } elseif ($row['Role'] == 'TL') {
                header('Location:../TL/index_tl.php');
            } else {
                // Handle other roles if necessary
                // For example, redirect to a generic dashboard
